...
If NO, then maybe you really do need to store a copy of a subset of the data.frame. But seriously consider whether you can achieve your goals by simply using the `subset =` argument -- or perhaps `with()` coupled with `subset()` -- to a computation to a specific set of rows. If this still does not suit your needs, then maybe you really should use `subset()` as shown above and carry on.
 ...
 * The heart of `plyr` is a set a functions with names like this:
```
XYply
```
   where `X` specifies what sort of input you're giving and `Y` specifies the sort of output you want.
...
Today we will emphasize `ddply()` which accepts a data.frame, splits it into pieces based on one or more factors, computes on them, then returns the results as a data.frame. For the record, the built-in functions most relevant to `ddply()` are `tapply()` and friends.
...
`summarize()` or its synonym `summarise()` is a function provided by `plyr` that creates a new data.frame from an old one. It is related to the built-in function `transform()` that transforms variables in a data.frame or adds new ones.
...
tail(jCoefs)
```
Finally, let's present this information attractively in a table. *I consider my advice about how to do this less definitive than the above. I'll be happy to see people explore other table-making tools.*
...
library(xtable)
```
Let's pick some countries at random and display their estimated coefficients.
...
tail(jCoefs)
```
Now we will remake the small table, starting with the country with the shortest life expectancy in 1952, going to the largest.
```{r results='asis', echo=FALSE}
set.seed(916)
foo <- jCoefs[sample(nrow(jCoefs), size = 15), ]
foo <- foo[order(foo$intercept), ]
foo <- xtable(foo)
print(foo, type='html', include.rownames = FALSE)
```
...
`ddply()` is the most important function, since data.frame are great, and it takes data.frame as input and returns data.frame as output.

Simple functions, built-in or user-defined, can be provided directly in a call to `ddply()`.

It's better to handle more complicated data aggregation differently. Build them up slowly and don't hesitate to revisit earlier steps for improvement. Here is a gentle workflow:
...
Results you present to the world generally need polishing to make the best impression and have the best impact. This too will be an iterative process. Here are some good things to think about:
  ...
  * Have I presented the data in the more aesthetically pleasing way I currently know how?
  
### Exporting data

What it you wanted to write the table of slopes and intercepts to file? We'll talk about that more soon, but the main functions to consider are `write.table()`, `dput()`, and `saveRDS()`.
