...
tail(jCoefs)
```
Now, prior to making the HTML table, we will sort the data.frame, so it starts with the country with the shortest life expectancy in 1952, and goes to the largest. *Thanks to student Viv for telling me to use `plyr` function `arrange()` here for a cleaner sorting command.*
```{r results='asis'}
set.seed(916)
foo <- jCoefs[sample(nrow(jCoefs), size = 15), ]
foo <- arrange(foo, intercept)
## foo <- foo[order(foo$intercept), ] # the older uglier way
foo <- xtable(foo)
print(foo, type='html', include.rownames = FALSE)
```
...
What it you wanted to write the table of slopes and intercepts to file? We'll talk about that more soon, but if this is an emergency, the main functions to consider are `write.table()`, `dput()`, and `saveRDS()`.

### References

`plyr` paper: [The split-apply-combine strategy for data analysis](http://www.jstatsoft.org/v40/i01/paper), Hadley Wickham, Journal of Statistical Software, vol. 40, no. 1, pp. 1–29, 2011. 
