...
```
Despite our focus on less data, we still have some overplotting. It can be nice to add jitter, which is just a small bit of meaningless noise, in the horizontal position, which corresponds to continent.
```{r}
...
```
I'm tired of dragging Oceania around, with its two measly countries. Let's remove all associated observations. To truly drop "Oceania" as a factor level, I use the `droplevels()` function (for more on being the boss of factors go FUTURE FACTOR TUTORIAL).
```{r}
...
stripplot(lifeExp ~ continent, iDat, jitter.data = TRUE,
          grid = "h", type = c("p", "a"))
```
I'd like the continents to be in order of life expectancy, low to high. And yes I realize that's an ill-defined thing. You will have to make lots of judgement calls to get anywhere in data analysis. Embrace being The Decider. (For more on reordering factors go FUTURE FACTOR TUTORIAL).
```{r}
...
Two arguments you may wish to tune on your densityplots are `bw =`, `adjust =`, and `n =`, which specify the bandwidth of the underlying kernel smoother and how fine a grid is used when the kernel density estimate is evaluated. It is really easy to confuse yourself here: believe it or not, `n = ` has nothing to do with sample size or the number of observations in your dataset. To make the underlying kernel density estimate smoother, increase the bandwidth. To make the connect-the-dots portrayal of it on your plot better able to follow the curve, whatever it is, increase `n`. Let's demonstrate.
...
bwplot(lifeExp ~ continent, gDat)
bwplot(lifeExp ~ continent, gDat, panel = panel.violin)
