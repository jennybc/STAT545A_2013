...
```{r}
foo <- ddply(gDat, ~ year, summarize, tMean = mean(lifeExp, trim = 0.2))
(foo <- arrange(foo, tMean))
```
...
ddply(gDat, ~ continent + year, function(x) sum(x$lifeExp < bMark))
```
