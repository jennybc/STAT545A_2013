...
```{r include = FALSE}
## WARNING: here lies a total hack!

## when I compile to HTML with my personal package fileherd, the intermediate
## Markdown files go into a scratch directory. The use of local static files,
## like this PNG, isn't handled (yet), so this workaround symlinks the PNG to
## the appropriate place. Another workaround, used elsewhere, is to place the
## PNG on the web somewhere.
file.symlink("ggplot2book-fig6.6.png", "_fileherd/herding/ggplot2book-fig6.6.png")
```
