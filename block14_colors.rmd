...
They fall into three classes. From top to bottom, they are
  ...
  * sequential: great for low-to-high things where one extreme is exciting and the other is boring, like p-values and correlations
  ...
  * diverging: great for things that range from "extreme and negative" to "extreme and positive", going through "non extreme and boring" along the way, such as t statistics and z scores
...
### Hexadecimal RBG color specification
...
`#rrbbgg`, where `rr`, `bb`, and `gg` refer to color intensity in the red, blue, and green channels, respectively. Each is specified as a two-digit base 16 number, which is the meaning of "hexadecimal" (or "hex" for short). Here's a table relating base 16 numbers to the beloved base 10 system.
```{r include = FALSE}
library(xtable)
...
print(foo, type='html')
```
Example: the first color in the palette is specified as "#1B9E77", so the intensity in the blue channel is 9E. What does that mean?
$$
9E = 9 * 16^1 + 14 * 16^0 = 9 * 16 + 14 = 158
$$
Note that the lowest possible channel intensity is `00` = 0 and the highest is `FF` = 255.

Important special cases that help you stay oriented. Here are the saturated primary colors, red, blue, and green:
```{r include = FALSE}
...
Earl F. Glynn (Stowers Institute for Medical Research)
  ...
  * [excellent color resources](http://research.stowers-institute.org/efg/R/Color/Chart/)
  * [talk "Using Color in R"](http://research.stowers-institute.org/efg/Report/UsingColorInR.pdf)
