...
The topic of factor level reordering will be covered elsewhere, but let's Just. Do. It. I reorder the country factor levels according to the intercept, i.e. the life expectancy in 1952.
...
head(jCoefs)
```
Note that the __row order of `gDat` has not changed__. I could choose to reorder the rows of the data.frame if I wanted, for example, if I was about to prepare a table to present to people. But I'm not, so I won't.

```{r eval = FALSE, include = FALSE}
# When I first attempted to reorder the levels of jCoefs$country, nothing
# happened. Much head-scratching, leading to the self-contained example below. I
# eventually determined that instead of calling reorder.default in stats I was
# getting reorder.factor in gdata. I didn't even know gdata was installed and
# loaded -- still not sure *why* gdata was loaded but whatever. Bottom line:
# reorder.factor does not have a default value of FUN whereas reorder.default
# does and it is mean(). I emailed the example below to the maintainer of gdata
# wondering if maybe FUN could have mean() as default.

# foo <- droplevels(subset(jCoefs,
#                          country %in% c("Gambia", "Mali", "China", "Cuba", "Norway")))
# head(levels(foo$country))
# dput(foo)
foo <- structure(list(country = structure(1:5, .Label = c("China", "Cuba", 
"Gambia", "Mali", "Norway"), class = "factor"), continent = structure(c(3L, 
2L, 1L, 1L, 4L), .Label = c("Africa", "Americas", "Asia", "Europe"
), class = "factor"), intercept = c(47.1904815384616, 62.2134487179487, 
28.4003717948718, 33.0512307692308, 72.2146153846154), slope = c(0.530714853146853, 
0.321150349650349, 0.581825874125873, 0.37680979020979, 0.131941258741258
)), .Names = c("country", "continent", "intercept", "slope"), row.names = c(25L, 
33L, 47L, 80L, 96L), class = "data.frame")
foo
levels(foo$country) # alphabetical order
library(gdata)
levels(reorder(foo$country, foo$intercept)) ## WHAT??
levels(reorder(foo$country, foo$intercept, mean))  ## yes!
levels(reorder(foo$country, foo$intercept, max))  ## yes!
levels(reorder(foo$country, foo$intercept, min))  ## yes!
```

### `saveRDS()` and `readRDS()`

If you have a data.frame AND you have exerted yourself to rationalize the factor levels, you have my blessing to save it to file in way that will preserve this hard work upon re-import. Use `saveRDS()`. Note that, unlike an approach based on `save()` and `load()`, you must make an object assignment upon import. This implies that you could give the object a totally different name, but in general, you will confuse yourself less if you keep using the same name. You will come across `save()` and `load()` which save and restore an entire workspace but I recommend you use them very sparingly, if at all. Source is real; workspaces are not.

```{r}
saveRDS(jCoefs, "jCoefs.rds")
```
This function has more arguments, such one controlling compression, so read the help for more advanced usage.
...
The main application of this is [the creation of highly portable, self-contained minimal examples](http://stackoverflow.com/questions/5963269/how-to-make-a-great-r-reproducible-example). For example, if you want to pose a question on a forum or directly to an expert, it might be required or just plain courteous to NOT attach any data files. You will need a monolithic, plain text blob that defines any necessary objects and has the necessary code. `dput()` can be helpful.
